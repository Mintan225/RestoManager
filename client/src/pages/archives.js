...
import { Archive, Euro, ShoppingCart, Trash2 } from "lucide-react";
      ...
      <Tabs defaultValue="orders" className="space-y-4">
          ...
          <Card>
            ...
            <CardContent>
              {ordersLoading ? (<div className="space-y-4">
                ...
                </div>) : deletedOrders.length > 0 ? (<div className="space-y-4">
                  {deletedOrders.map((order) => {
                var _a;
                return (<div key={order.id} className="flex items-center justify-between py-3 border-b last:border-b-0 bg-gray-50 p-4 rounded-lg">
                      <div className="flex-1">
                        ...
                        <div className="mt-2 flex items-center gap-4">
                          ...
                          {order.customerName && (<span className="text-sm text-gray-600">
                              Client: {order.customerName}
                            </span>)}
                          {order.tableId && (<span className="text-sm text-gray-600">
                              Table: {order.tableId}
                            </span>)}
                        </div>
                        {((_a = order.orderItems) === null || _a === void 0 ? void 0 : _a.length) > 0 && (<div className="mt-2 text-sm text-gray-600">
        ...
        <TabsContent value="sales">
          <Card>
            ...
            <CardContent>
              {salesLoading ? (<div className="space-y-4">
                  {[1, 2, 3].map((i) => (<div key={i} className="animate-pulse">
                      <div className="h-4 bg-gray-200 rounded w-3/4 mb-2"></div>
                      <div className="h-3 bg-gray-200 rounded w-1/2"></div>
                    </div>))}
                </div>) : deletedSales.length > 0 ? (<div className="space-y-4">
                  {deletedSales.map((sale) => (<div key={sale.id} className="flex items-center justify-between py-3 border-b last:border-b-0 bg-gray-50 p-4 rounded-lg">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3">
                          <div>
                            <p className="font-medium text-gray-900">
        ...
        <TabsContent value="expenses">
          <Card>
            ...
            <CardContent>
              {expensesLoading ? (<div className="space-y-4">
                  {[1, 2, 3].map((i) => (<div key={i} className="animate-pulse">
                      <div className="h-4 bg-gray-200 rounded w-3/4 mb-2"></div>
                      <div className="h-3 bg-gray-200 rounded w-1/2"></div>
                    </div>))}
                </div>) : deletedExpenses.length > 0 ? (<div className="space-y-4">
                  {deletedExpenses.map((expense) => (<div key={expense.id} className="flex items-center justify-between py-3 border-b last:border-b-0 bg-gray-50 p-4 rounded-lg">
                      <div className="flex-1">
                        <div className="flex items-center space-x-3">
                          <div>
                            <p className="font-medium text-gray-900">
