...
export default function NotificationDemo() {
    ...
    const showNotification = (order) => {
        const notification = Object.assign(Object.assign({}, order), { notificationId: `${order.id}-${Date.now()}` });
    ...
    return (<div className="min-h-screen bg-gray-50 p-6">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
          <Card>
            ...
            <CardContent className="space-y-4">
              {demoOrders.map((order) => (<div key={order.id} className="flex items-center justify-between p-3 border rounded">
                  <div>
                  ...
                  <Button onClick={() => showNotification(order)} size="sm">
                    Afficher
                  </Button>
                </div>))}
            </CardContent>
        ...
        <div className="mt-8 text-center">
          <Button onClick={() => setActiveNotifications([])} variant="outline">
      ...
      {activeNotifications.map((notification) => (<OrderNotification key={notification.notificationId} order={notification} onClose={() => removeNotification(notification.notificationId)}/>))}
    </div>);
}
