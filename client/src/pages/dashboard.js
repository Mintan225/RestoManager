...
import { Plus, Package, QrCode, Receipt } from "lucide-react";
...
export default function Dashboard() {
    ...
    const occupiedTableIds = new Set(activeOrders
        .filter((order) => order.status !== 'completed' && order.status !== 'cancelled')
        .map((order) => order.tableId));
    const occupiedTablesFromOrders = occupiedTableIds.size;
    const occupiedTablesFromStatus = tables.filter((t) => t.status === 'occupied').length;
    ...
    const stats = {
        todaySales: (todayStats === null || todayStats === void 0 ? void 0 : todayStats.totalSales) || 0,
        activeOrders: activeOrders.length || 0,
        occupiedTables: `${occupiedTablesCount}/${tables.length}`,
        todayExpenses: (todayStats === null || todayStats === void 0 ? void 0 : todayStats.totalExpenses) || 0,
        todayProfit: (todayStats === null || todayStats === void 0 ? void 0 : todayStats.profit) || 0,
    };
    // Données réelles des ventes hebdomadaires
    const weeklyData = weeklyStats.map((stat, index) => ({
        day: stat.day,
        sales: stat.sales,
        orders: stat.orders,
    ...
    return (<div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <Card>
          ...
          <CardContent>
            {ordersLoading ? (<div className="space-y-4">
              ...
              </div>) : activeOrders.length > 0 ? (<div className="space-y-4">
                {activeOrders.slice(0, 3).map((order) => (<div key={order.id} className="flex items-center justify-between py-2 border-b last:border-b-0">
        ...
        <Card>
          ...
          <CardContent>
            {weeklyLoading ? (<div className="space-y-4">
                {[1, 2, 3, 4, 5, 6, 7].map((i) => (<div key={i} className="animate-pulse flex items-center">
                    <div className="w-12 h-4 bg-gray-200 rounded"></div>
                    <div className="flex-1 bg-gray-200 rounded-full h-4 mx-2"></div>
                    <div className="w-16 h-4 bg-gray-200 rounded"></div>
                  </div>))}
              </div>) : weeklyData.length > 0 ? (<div className="space-y-4">
                {weeklyData.map((data, index) => (<div key={index} className="flex items-center">
                    <div className="w-12 text-sm text-gray-500">{data.day}</div>
                    <div className="flex-1 bg-gray-200 rounded-full h-4 mx-2">
                      <div className={`${data.color} h-4 rounded-full transition-all duration-300`} style={{
