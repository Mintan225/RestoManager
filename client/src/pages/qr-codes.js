...
export default function QRCodes() {
    ...
    const createTableMutation = useMutation({
        mutationFn: (tableData) => __awaiter(this, void 0, void 0, function* () {
            const qrData = generateTableQRData(parseInt(tableData.number));
            const response = yield fetch("/api/tables", {
                method: "POST",
                headers: authService.getAuthHeaders(),
                body: JSON.stringify(Object.assign(Object.assign({}, tableData), { qrCode: qrData })),
            });
            if (!response.ok) {
    ...
    const regenerateAllMutation = useMutation({
        mutationFn: () => __awaiter(this, void 0, void 0, function* () {
            ...
            const response = yield fetch("/api/admin/regenerate-qr-codes", {
                method: "PUT",
                headers: authService.getAuthHeaders(),
            });
            if (!response.ok) {
    ...
    const downloadAllQRCodes = () => __awaiter(this, void 0, void 0, function* () {
        try {
            for (const table of tables) {
                const { generateTableQRCode, downloadQRCode } = yield import("@/lib/qr-utils");
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex space-x-2">
          <Button onClick={downloadAllQRCodes} variant="outline" disabled={tables.length === 0}>
            <Download className="h-4 w-4 mr-2"/>
            Télécharger tout
          </Button>
          <Button onClick={() => regenerateAllMutation.mutate()} variant="outline" disabled={regenerateAllMutation.isPending || tables.length === 0}>
            ...
            <DialogContent>
              ...
              <div className="space-y-4">
                <div className="space-y-2">
                  ...
                  <Input id="tableNumber" type="number" value={newTableNumber} onChange={(e) => setNewTableNumber(e.target.value)} placeholder="1" min="1"/>
                </div>
                <div className="space-y-2">
                  <Label htmlFor="tableCapacity">Capacité (nombre de personnes)</Label>
                  <Input id="tableCapacity" type="number" value={newTableCapacity} onChange={(e) => setNewTableCapacity(e.target.value)} placeholder="4" min="1"/>
                </div>
      ...
      {isLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        </div>) : tables.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {tables
                .sort((a, b) => a.number - b.number)
                .map((table) => (<QRGenerator key={table.id} table={table}/>))}
        </div>) : (<Card>
