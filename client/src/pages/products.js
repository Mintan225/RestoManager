...
export default function Products() {
    ...
    const deleteMutation = useMutation({
        mutationFn: (productId) => __awaiter(this, void 0, void 0, function* () {
            const response = yield fetch(`/api/products/${productId}`, {
                method: "DELETE",
    ...
    const getCategoryName = (categoryId) => {
        ...
        return (category === null || category === void 0 ? void 0 : category.name) || "Sans catégorie";
    };
    return (<div className="space-y-6">
      ...
      {isLoading ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        ...
        </div>) : filteredProducts.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredProducts.map((product) => (<Card key={product.id} className="overflow-hidden">
              ...
              <CardContent>
                {product.description && (<p className="text-sm text-gray-600 mb-4 line-clamp-2">
                    {product.description}
                  </p>)}
                <div className="flex items-center justify-between">
