...
export function OrderTracking({ tableId, customerName, customerPhone, onClose }) {
    ...
    const orders = (menuData === null || menuData === void 0 ? void 0 : menuData.orders) || [];
    ...
    const customerOrders = orders.filter((order) => {
        ...
        if (customerName && customerPhone) {
            const nameMatch = ((_a = order.customerName) === null || _a === void 0 ? void 0 : _a.toLowerCase()) === customerName.toLowerCase();
    ...
    const activeOrders = customerOrders.filter((order) => order.status !== 'completed');
      ...
      <Card className="w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          ...
          <div className="flex items-center space-x-2">
            <Button variant="ghost" size="sm" onClick={() => setAutoRefresh(!autoRefresh)} className={autoRefresh ? "text-green-600" : "text-gray-400"}>
        ...
        <CardContent className="space-y-4">
          {isLoading ? (<div className="text-center py-8">
            ...
            </div>) : activeOrders.length === 0 ? (<div className="text-center py-8">
            ...
            return (<Card key={order.id} className="border-l-4" style={{ borderLeftColor: statusInfo.color.replace('bg-', '#') }}>
                  <CardContent className="p-4">
                    ...
                    <div className="mb-3">
                      ...
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div className={`h-2 rounded-full transition-all duration-500 ${statusInfo.color}`} style={{ width: `${statusInfo.progress}%` }}></div>
                      </div>
                    </div>

                    <p className="text-sm text-gray-700 mb-3">{statusInfo.description}</p>
                    
                    {order.customerName && (<p className="text-sm text-gray-600 mb-2">
                        Client: {order.customerName}
                      </p>)}

                    {/* Détails des articles */}
                    <div className="space-y-1 mb-3">
                      {(_a = order.orderItems) === null || _a === void 0 ? void 0 : _a.map((item) => (<div key={item.id} className="flex justify-between text-sm">
                    ...
                    {order.status === 'ready' && (<div className="mt-3 p-2 bg-green-50 border border-green-200 rounded text-sm text-green-800">
