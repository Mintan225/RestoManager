...
import { Settings, CreditCard, Smartphone, Building2, Globe, Phone, Mail, MapPin, Check, X, TestTube } from "lucide-react";
// Importation du service d'authentification manquant
import authService from "@/lib/auth";
export default function Config() {
    const [testingPayment, setTestingPayment] = useState(null);
    ...
    const testPaymentMethod = (method) => __awaiter(this, void 0, void 0, function* () {
        setTestingPayment(method);
        try {
            const response = yield fetch("/api/payments/initiate", {
                method: "POST",
    ...
    return (<div className="container mx-auto py-8 space-y-6">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.restaurant.NAME}</p>
            </div>
            <div className="space-y-2">
              ...
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.restaurant.ADDRESS}</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium flex items-center gap-1">
              ...
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.restaurant.PHONE}</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium flex items-center gap-1">
              ...
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.restaurant.EMAIL}</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium flex items-center gap-1">
              ...
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.restaurant.WEBSITE}</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {config === null || config === void 0 ? void 0 : config.paymentMethods.map((method) => (<div key={method.id} className="flex items-center justify-between p-4 border rounded-lg">
                ...
                <div className="flex items-center gap-2">
                  <Badge variant={method.enabled ? "default" : "secondary"}>
                    {method.enabled ? (<Check className="h-3 w-3 mr-1"/>) : (<X className="h-3 w-3 mr-1"/>)}
                    {method.enabled ? "Activé" : "Désactivé"}
                  </Badge>
                  {method.enabled && method.id !== "cash" && (<Button size="sm" variant="outline" onClick={() => testPaymentMethod(method.id)} disabled={testingPayment === method.id}>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-2">
              ...
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.business.TAX_RATE}%</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Frais de service</label>
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.business.SERVICE_CHARGE}%</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Frais de livraison</label>
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.business.DELIVERY_FEE} FCFA</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Commande minimum</label>
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.business.MINIMUM_ORDER} FCFA</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              ...
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.app.name}</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Version</label>
              <p className="text-sm bg-gray-50 p-2 rounded">{config === null || config === void 0 ? void 0 : config.app.version}</p>
            </div>
            <div className="space-y-2">
              <label className="text-sm font-medium">Environnement</label>
              <Badge variant={(config === null || config === void 0 ? void 0 : config.app.environment) === "production" ? "default" : "secondary"}>
                {config === null || config === void 0 ? void 0 : config.app.environment}
              </Badge>
            </div>
          </div>
        </CardContent>
