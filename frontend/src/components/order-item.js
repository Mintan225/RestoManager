...
export function OrderItem({ order }) {
    ...
    const updateOrderMutation = useMutation({
        mutationFn: (_a) => __awaiter(this, [_a], void 0, function* ({ status }) {
            const response = yield fetch(`/api/orders/${order.id}`, {
                method: "PUT",
        ...
        onSuccess: () => {
            ...
            queryClient.invalidateQueries({
                predicate: (query) => {
                    var _a;
                    return (_a = query.queryKey[0]) === null || _a === void 0 ? void 0 : _a.toString().startsWith('/api/menu/');
                }
            });
        },
        onError: (error) => {
      ...
      <CardContent className="p-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
          <div>
            ...
            {order.customerName && (<p className="text-sm text-gray-600">
                <strong>Client:</strong> {order.customerName}
              </p>)}
            {order.customerPhone && (<p className="text-sm text-gray-600">
                <strong>Téléphone:</strong> {order.customerPhone}
              </p>)}
          </div>
          <div>
            {order.paymentMethod && (<p className="text-sm text-gray-600">
                <strong>Paiement:</strong> {order.paymentMethod}
              </p>)}
            {order.notes && (<p className="text-sm text-gray-600">
        ...
        {/* Produits commandés */}
        {order.orderItems && order.orderItems.length > 0 && (<div className="mb-4">
            <h4 className="font-medium text-sm text-gray-700 mb-2">Produits commandés :</h4>
            <div className="space-y-2">
              {order.orderItems.map((item) => (<div key={item.id} className="flex justify-between items-center p-2 bg-gray-50 rounded">
        ...
        <div className="flex flex-wrap gap-2 items-center justify-between">
          {order.status !== "completed" && order.status !== "cancelled" && (<div className="flex space-x-2">
              {order.status === "pending" && (<Button size="sm" onClick={() => handleStatusChange("preparing")} disabled={updateOrderMutation.isPending}>
                  Commencer préparation
                </Button>)}
              {order.status === "preparing" && (<Button size="sm" onClick={() => handleStatusChange("ready")} disabled={updateOrderMutation.isPending}>
                  Marquer comme prêt
                </Button>)}
              {order.status === "ready" && (<Button size="sm" onClick={() => handleStatusChange("completed")} disabled={updateOrderMutation.isPending}>
                  Terminer
                </Button>)}
              <Button size="sm" variant="destructive" onClick={() => handleStatusChange("cancelled")} disabled={updateOrderMutation.isPending}>
                Annuler
              </Button>
            </div>)}
          
          {/* Bouton de téléchargement du reçu - disponible pour toutes les commandes payées */}
          {order.paymentStatus === "paid" && (<Button size="sm" variant="outline" onClick={handleDownloadReceipt} className="flex items-center gap-1">
